...
class LuaFilePathViewer(QMainWindow):
    ...
    def update_progress_label(self, current, total, current_file_path=None):
        if current_file_path:
            self.progress_label.setText(f"进度: {current}/{total} - 当前文件: {current_file_path}")  # 更新进度标签
        else:
            self.progress_label.setText(f"进度: {current}/{total}")  # 更新进度标签
    ...
    def start_processing(self):
        ...
        for index, file_path in enumerate(self.file_paths):
            ...
            # 更新进度标签为当前进度和正在处理的文件名
            self.update_progress_label(index + 1, total_files, file_name)  # 传递文件名
    ...
    def stop_processing(self):
        ...
        if self.current_process:  # 确保当前进程不为 None
            self.current_process.terminate()  # 停止当前运行的进程
